...
This rule reports ES2025 [`Iterator.prototype.every` method](https://github.com/tc39/proposal-iterator-helpers) as errors.
...
```js
/*eslint es-x/no-iterator-prototype-every: [error, { aggressive: true }] */
Iterator.from([]).every(a => !!a)
```
