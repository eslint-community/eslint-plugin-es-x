"use strict"

const path = require("path")
const RuleTester = require("../../tester")
const rule = require("../../../lib/rules/no-iterator-prototype-map.js")
const ruleId = "no-iterator-prototype-map"

new RuleTester().run(ruleId, rule, {
    valid: [
        "map(String)",
        "foo.unknown(0)",
        "foo.map(String)",
        {
            code: "map(String)",
            settings: { "es-x": { aggressive: true } },
        },
        { code: "foo.unknown(0)", settings: { "es-x": { aggressive: true } } },
        {
            code: "foo.map(String)",
            options: [{ aggressive: false }],
            settings: { "es-x": { aggressive: true } },
        },
    ],
    invalid: [
        {
            code: "Iterator.from([]).map(String)",
            errors: ["ES2025 'Iterator.prototype.map' method is forbidden."],
        },
        {
            code: "foo.map(String)",
            errors: ["ES2025 'Iterator.prototype.map' method is forbidden."],
            settings: { "es-x": { aggressive: true } },
        },
        {
            code: "foo.map(String)",
            options: [{ aggressive: true }],
            errors: ["ES2025 'Iterator.prototype.map' method is forbidden."],
            settings: { "es-x": { aggressive: false } },
        },
    ],
})

// -----------------------------------------------------------------------------
// TypeScript
// -----------------------------------------------------------------------------
const parser = require("@typescript-eslint/parser")
const tsconfigRootDir = path.resolve(__dirname, "../../fixtures")
const project = "tsconfig.json"
const filename = path.join(tsconfigRootDir, "test.ts")

new RuleTester({
    languageOptions: { parser, parserOptions: { tsconfigRootDir, project } },
}).run(`${ruleId} TS Full Type Information`, rule, {
    valid: [
        { filename, code: "map(String)" },
        { filename, code: "foo.unknown(0)" },
        { filename, code: "foo.map(String)" },
        {
            filename,
            code: "let foo = {}; foo.map(String)",
        },
        {
            filename,
            code: "map(String)",
            settings: { "es-x": { aggressive: true } },
        },
        {
            filename,
            code: "foo.unknown(0)",
            settings: { "es-x": { aggressive: true } },
        },
    ],
    invalid: [
        {
            filename,
            code: "let foo = Iterator.from([]); foo.map(String)",
            errors: ["ES2025 'Iterator.prototype.map' method is forbidden."],
        },
        {
            filename,
            code: "function f(a: Iterator) { a.map(String) }",
            errors: ["ES2025 'Iterator.prototype.map' method is forbidden."],
        },
        {
            filename,
            code: "foo.map(String)",
            errors: ["ES2025 'Iterator.prototype.map' method is forbidden."],
            settings: { "es-x": { aggressive: true } },
        },
    ],
})
