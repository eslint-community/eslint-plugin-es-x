...
const aggregateErrorTraceMap = {
    AggregateError: { [CONSTRUCT]: true, [READ]: true },
}
...
function isConstructCallWithCauseOption(node, isAggregateError) {
    ...
    if (isSpreadElement(node.arguments[0])) {
        return false
    }
    if (isAggregateError && isSpreadElement(node.arguments[1])) {
        return false
    }
    const optionsArg = node.arguments[isAggregateError ? 2 : 1]
    ...
    create(context) {
        ...
        return {
            ...
            "Program:exit"() {
                ...
                function reportError(isAggregateError) {
                    const traceMap = isAggregateError
                        ? aggregateErrorTraceMap
                        : errorsTraceMap
                ...
                reportError(/* isAggregateError */ false)
                reportError(/* isAggregateError */ true)
            },
        }
    },
}
