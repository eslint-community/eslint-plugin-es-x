...
module.exports = { createGuardsContext }
 ...
 */
function createGuardsContext(options) {
     ...
     * @returns {((node: MemberExpression|Property)=>boolean)|null} The guard checker.
     */
    ...
    function getGuardChecker(node, propertyTypes) {
        ...
        if (propertyTypes.includes("undefined")) {
            ...
            // it is difficult to determine whether the property access is guarded and
            // therefore cannot be excluded from the check.
            return null
        }
        if (node.type !== "MemberExpression") {
            return false
        ...
        if (parent.type === "BinaryExpression") {
            ...
            if (staticValue.value === undefined) {
                if (parent.operator === "!==" || parent.operator === "!=") {
                    ...
                    return getGuardCheckerForExpression(parent)
                } else if (
                    parent.operator === "===" ||
                    parent.operator === "=="
                ) {
                ...
                if (parent.operator === "!=") {
                    ...
                    // e.g. if (x.property == null)
                    return getGuardCheckerForExpression(parent, { not: true })
                }
            }
            return null
        }
        if (parent.type === "UnaryExpression" && parent.operator === "typeof") {
            ...
            if (pp.operator === "!==" || pp.operator === "!=") {
                if (type === "undefined") {
                    ...
                    return getGuardCheckerForExpression(pp)
                }
                // e.g. if (typeof x.property !== "function"), if (typeof x.property != "function")
                return getGuardCheckerForExpression(pp, { not: true })
        ...
        if (
            ...
            // e.g. if (x.property)
            return getGuardCheckerForExpression(node)
        }
        return null
    }
    ...
    function getGuardCheckerForUnknownType(node) {
        ...
        // eslint-disable-next-line complexity
        return (n) => {
            if (primitiveChecker?.(n)) {
                return true
            }
            const maybeMethod = methodChecker?.(n)
            if (maybeMethod) {
                ...
                // it cannot be checked because the property may have a falsy value.
            }
            if (maybeMethod || objectChecker?.(n)) {
                const parent = getParent(n)
                if (
                ...
                // If the property is not property access,
                // it cannot be checked because the property may have a falsy value.
            }
            return false
        }
