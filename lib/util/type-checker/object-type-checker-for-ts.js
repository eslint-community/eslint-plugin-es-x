 ...
 * @returns {((memberAccessNode: MemberExpression|Property, objectNode: Expression, className: string) => boolean | "aggressive") | null} Returns an object type checker. Returns null if TypeScript is not available.
 */
function buildObjectTypeCheckerForTS(context, aggressiveResult) {
    ...
    return function (memberAccessNode, objectNode, className) {
        return (
     ...
     * @param {MemberExpression|Property} memberAccessNode The MemberExpression or Property node.
     ...
     * @param {Expression} objectNode The Expression node.
     * @param {string} className The class name to disallow.
     * @returns {boolean} `true` if should disallow it.
     */
    function checkByObjectExpressionType(objectNode, className) {
