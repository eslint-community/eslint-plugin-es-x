...
function buildExpressionTypeProviderImpl(context) {
    ...
    function getIdentifierTypeInfo(node) {
        ...
        if (def.type === "Variable") {
            return getVariableDeclaratorTypeInfo(variable)
    ...
    /**
     * @param {import("eslint").Scope.Variable & {defs:[Extract<import("eslint").Scope.Definition, {type:'Variable'}>]}} variable
     * @returns {TypeInfo | null}
     */
    // eslint-disable-next-line complexity
    function getVariableDeclaratorTypeInfo(variable) {
        const def = variable.defs[0]
        if (
