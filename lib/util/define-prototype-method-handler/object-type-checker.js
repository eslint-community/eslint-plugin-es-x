...
const { WELLKNOWN_GLOBALS, WELLKNOWN_PROTOTYPE } = require("./es-types")
...
function buildExpressionTypeProviderImpl(context) {
    ...
    function getIdentifierTypeInfo(node) {
        ...
        if (def.type === "Variable") {
            if (
                ...
                return getTypeInfo(def.node.init)
            }
        ...
        return {
            ...
            get return() {
                ...
                for (const st of iterateReturn(node.body.body)) {
                    ...
                    const typeInfo = getTypeInfo(st.argument)
                    if (!returnStatement) {
                        returnStatement = typeInfo
                    } else if (returnStatement.type === typeInfo.type) {
                        /** @type {TypeInfo} */
                        ...
                        returnStatement = {
                            ...
                            get return() {
                                ...
                                const type2 = typeInfo.return?.type
    ...
    function getMemberExpressionTypeInfo(node) {
        ...
        const info = getTypeInfo(node.object)
        if (!info) {
            return null
        }
        const prop = info.properties?.[propName]
        if (prop) {
            return prop
        }
        const proto = WELLKNOWN_PROTOTYPE[info.type]
        return proto?.[propName] ?? null
    }
}
